...
from doomsday.date import find_if_is_bissextile
...
def get_operations_on_end_year(years: str) -> int:
    """Do the preliminary operations on the last 2  digits of year"""
...
def add_11_if_number_is_odd(number: int) -> int:
    """Add eleven if the number given is odd"""
...
def get_superior_multiple_of_seven(number: int) -> int:
    """Get the next multiple of 7 above or equal to given number"""
...
def get_position_key_day_in_month(month: str, year: str) -> int:
    ...
    if int_month < 3:
        if find_if_is_bissextile(int(year)):
